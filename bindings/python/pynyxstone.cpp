...
std::vector<uint8_t> assemble_to_bytes(
    Nyxstone& nyxstone, std::string assembly, uint64_t address, std::vector<Nyxstone::LabelDefinition> labels)
{
...
PYBIND11_MODULE(nyxstone, m)
{
    ...
    py::class_<Nyxstone>(m, "Nyxstone")
        ...
        .def("assemble_to_instructions", &assemble_to_instructions, py::arg("assembly"), py::arg("address"),
            py::arg("labels"))
        .def("disassemble_to_instructions", &disassemble_to_instructions, py::arg("bytes"), py::arg("address"),
            py::arg("count"))
